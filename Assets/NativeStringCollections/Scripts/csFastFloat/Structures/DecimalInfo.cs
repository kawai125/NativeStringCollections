...
namespace NativeStringCollections.Impl.csFastFloat.Constants
{
    ...
    internal struct DecimalInfo
    {
        ...
        private static byte number_of_digits_decimal_left_shift_table_powers_of_5(int index)
        {
            ...
            CheckNumberOfDigitsDecimalLeftShiftTablePowersOf5(index);
            return number_of_digits_decimal_left_shift_table_powers_of_5_table[(uint)index];
        }
        [Conditional("ENABLE_UNITY_COLLECTIONS_CHECKS")]
        private static void CheckNumberOfDigitsDecimalLeftShiftTablePowersOf5(int index)
        {
            var table = number_of_digits_decimal_left_shift_table_powers_of_5_table;
            if (index >= table.Length)
                throw new ArgumentOutOfRangeException($"index = {index}. range: [0, {table.Length - 1}]");
        }

        private static readonly byte[] number_of_digits_decimal_left_shift_table_powers_of_5_table = new byte[] {
        ...
        // UTF-16 inputs
        unsafe internal static DecimalInfo parse_decimal(Char16* p, Char16* pend, Char16 decimal_separator)
        {
