...
{
    public static unsafe class FastFloatParser
    {
        ...
        internal static bool TryParseNumber(Char16* first, Char16* last, out int characters_consumed, out float result, UInt16 decimal_separator = UTF16CodeSet.code_dot)
        {
        ...
        internal static bool TryHandleInvalidInput(Char16* first, Char16* last, out int characters_consumed, out float result)
        {
            result = 0;
            characters_consumed = 0;
            ...
            if (last - first >= 3)
            {
                if (Utils.strncasecmp(first, Utils.strncasecmpMatch.nan))
                {
                    ...
                    result = FloatBinaryConstants.NaN;
                    return true;
                }
                ...
                if (last - first >= 4)
                {
                        ...
                        Utils.strncasecmp(first, Utils.strncasecmpMatch.minus_inf))
                    {
                        ...
                        result = (first[0] == '-') ? FloatBinaryConstants.NegativeInfinity : FloatBinaryConstants.PositiveInfinity;
                        return true;
                    }
                }
            }
